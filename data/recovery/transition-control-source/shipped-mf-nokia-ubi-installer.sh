...
export PATH
umask 077
PRELOADER=/installer/openwrt-airoha-an7583-nokia_xg-040g-mf-ubi-preloader.bin
FIP=/installer/openwrt-airoha-an7583-nokia_xg-040g-mf-ubi-bl31-uboot.fip
PRELOADER_SHA='778d10a65276085b70bec005248fc87ec208b43b0239502f15ade20fe528301e'
FIP_SHA='99b6c20a7cb46a56692eaeb9f086f70fc7e987a641396653e6a8fb5c03e07aa7'
PRELOADER_SIZE=118333
FIP_SIZE=319568
BL2_BUILDSET_ID='mf-an7583-ubi-preloader-778d10a6-fip-99b6c20a'
SYSUPGRADE_SHA='db881b8053cdfbdf49dd6c2336dee3ddfa489966456a3e75556c5a0f6cc7663b'
DEFAULT_SYSUPGRADE='openwrt-airoha-an7583-nokia_xg-040g-mf-ubi-squashfs-sysupgrade.itb'
EMBEDDED_OPENWRT_SYSUPGRADE='/tmp/nokia-embedded-openwrt-sysupgrade.itb'
EMBEDDED_OPENWRT_SYSUPGRADE_SIZE=9191705
EMBEDDED_OPENWRT_SYSUPGRADE_OFFSET=0x8c0000
EMBEDDED_OPENWRT_SYSUPGRADE_SHA='db881b8053cdfbdf49dd6c2336dee3ddfa489966456a3e75556c5a0f6cc7663b'
BL2_SIZE=131072
BL2_OFFSET=2048
FF_2K_SHA='d0ff1b294b5288d1ae1421eadf5b2d38a8752b76d472ff30bed9028e25b1c5b8'
BL2_EXPECTED_SHA='c655479c4d14b4f6d1a7a5eb8de80bfb204b3cdf46c9e48a5c6f3aea22d98131'
...
log() {
    mkdir -p /www; printf '%s\n' "$*" | tee -a "$LOG" /www/medveflasher-transition.log
...
check_board() {
    ...
    [ "$(cat /tmp/sysinfo/board_name)" = 'nokia,xg-040g-mf-ubi' ] || \
        die "unexpected board: $(cat /tmp/sysinfo/board_name)"
    tr '\000' '\n' < /proc/device-tree/compatible 2>/dev/null | grep -qx 'nokia,xg-040g-mf-ubi' || \
        die 'unexpected device-tree compatible'
    [ -e /proc/device-tree/medveflasher,transition-writable-bl2 ] || \
        die 'transition-only writable-BL2 DT marker is missing'
    log '[OK] board=nokia,xg-040g-mf-ubi; transition DT marker present'
}
check_nand() {
    ...
    if [ -r /tmp/NOKIA_NAND_IDENTITY ]; then
        ...
        case "$cached" in
            gigadevice) log '[OK] GigaDevice MF NAND identity retained from the pre-format check.'; return 0 ;;
            blocked) die 'unexpected non-MF NAND identity retained from pre-format check' ;;
    ...
    for file in /sys/class/mtd/mtd*/device/name /sys/class/mtd/mtd*/device/model /sys/class/mtd/mtd*/device/modalias /sys/class/mtd/mtd*/device/uevent /sys/bus/spi/devices/*/modalias /sys/bus/spi/devices/*/uevent /proc/nand /proc/spi_nand /proc/driver/nand /proc/driver/spi_nand; do
        [ -r "$file" ] || continue
        printf '
--- %s ---
' "$file" >> "$evidence"
    ...
    if grep -qi -E 'gigadevice|gd5f2gm7|gd5f2g' "$evidence"; then
        identity=gigadevice; printf '%s
' "$identity" > /tmp/NOKIA_NAND_IDENTITY
        log '[OK] GigaDevice MF-family SPI-NAND explicitly detected.'; return 0
    fi
    if grep -qi -E 'skyhigh|ml02g300whi00|fudan|fm25g02b' "$evidence"; then
        identity=blocked; printf '%s
' "$identity" > /tmp/NOKIA_NAND_IDENTITY
        die 'unexpected non-MF NAND model detected'
    fi
    identity=unknown; printf '%s
' "$identity" > /tmp/NOKIA_NAND_IDENTITY
    log 'WARNING: kernel did not expose the SPI-NAND model string; exact MF board/geometry gates passed.'
}
check_geometry() {
    local bl2 ubi
    require_mtd all_flash 268435456
    require_mtd bl2 131072
    require_mtd ubi 268304384
    bl2="$(mtd_name bl2)"
    ubi="$(mtd_name ubi)"
    [ "$(cat "/sys/class/mtd/$bl2/erasesize")" = 131072 ] || die 'unexpected BL2 erase size'
    [ "$(cat "/sys/class/mtd/$ubi/erasesize")" = 131072 ] || die 'unexpected NAND erase size'
    [ "$(cat "/sys/class/mtd/$ubi/writesize")" = 2048 ] || die 'unexpected NAND write size'
    log '[OK] MF NAND geometry: physical=268435456 erase=131072 write=2048; bl2=0x20000 ubi=0xffe0000.'
}
check_tools() {
    local cmd
...
check_payloads() {
    ...
    [ -s "$FIP" ] || die 'MF FIP payload is missing'
    ...
    [ "$preloader_size" -eq "$PRELOADER_SIZE" ] || die "unexpected pinned preloader size: $preloader_size"
    [ "$fip_size" -eq "$FIP_SIZE" ] || die "unexpected pinned FIP size: $fip_size"
    ...
    log "[OK] MF FIP size=$fip_size SHA256=$FIP_SHA"
}
check_transition_bl2_write_gate() {
    local idx flags
    idx="$(mtd_name bl2)"
    [ -n "$idx" ] || die 'BL2 MTD is missing'
    flags="$(cat "/sys/class/mtd/$idx/flags" 2>/dev/null || true)"
    case "$flags" in
        0x*|[0-9]*) ;;
        *) die "cannot read BL2 MTD flags: ${flags:-missing}" ;;
    esac
    [ $((flags & 0x400)) -ne 0 ] || die 'transition DT leaves BL2 read-only; refusing to format UBI'
    log "[OK] transition-only BL2 write gate: $idx flags=$flags includes MTD_WRITEABLE."
}
verify_pinned_bl2_before_write() {
    local current prefix payload preloader_size payload_blocks fip_now
    [ -f "$BL2_IMAGE" ] || critical 'prepared BL2 image disappeared before final write'
    [ "$(file_size "$BL2_IMAGE")" -eq "$BL2_SIZE" ] || critical 'prepared BL2 image size changed before final write'
    [ "$(file_size "$PRELOADER")" -eq "$PRELOADER_SIZE" ] || critical 'pinned preloader size changed before final BL2 write'
    [ "$(sha_file "$PRELOADER")" = "$PRELOADER_SHA" ] || critical 'pinned preloader SHA256 changed before final BL2 write'
    [ "$(file_size "$FIP")" -eq "$FIP_SIZE" ] || critical 'pinned FIP size changed before final BL2 write'
    fip_now="$(sha_file "$FIP")"
    [ "$fip_now" = "$FIP_SHA" ] || critical 'pinned FIP SHA256 changed before final BL2 write'
    current="$(sha_file "$BL2_IMAGE")"
    [ "$current" = "$BL2_EXPECTED_SHA" ] || critical 'prepared BL2 image does not match the pinned MF build-set immediately before write'
    prefix="$(dd if="$BL2_IMAGE" bs="$BL2_OFFSET" count=1 2>/dev/null | sha256sum | awk '{print $1}')"
    [ "$prefix" = "$FF_2K_SHA" ] || critical 'prepared BL2 prefix changed before final write'
    preloader_size="$(file_size "$PRELOADER")"
    payload_blocks=$(( (preloader_size + BL2_OFFSET - 1) / BL2_OFFSET ))
    payload="$(dd if="$BL2_IMAGE" bs="$BL2_OFFSET" skip=1 count="$payload_blocks" 2>/dev/null | head -c "$preloader_size" | sha256sum | awk '{print $1}')"
    [ "$payload" = "$PRELOADER_SHA" ] || critical 'prepared BL2 payload no longer contains the exact pinned preloader'
    log "[OK] pinned BL2 provenance rechecked immediately before write: buildset=$BL2_BUILDSET_ID SHA256=$current"
}
prepare_bl2_image() {
...
attach_existing() {
    ...
    ibu="$(mtd_dev ubi)" || die 'ibu device is missing'
    ibu_num="$(mtd_num ubi)" || die 'cannot resolve ibu MTD number'
...
run_install() {
    ...
    ibu="$(mtd_dev ubi)" || critical 'ibu device disappeared'
    ibu_num="$(mtd_num ubi)" || critical 'cannot resolve ibu MTD number'
    ...
    log '[5/8] Writing MF FIP and fallback installer FIT.'
    ...
    verify_pinned_bl2_before_write
    log '[7/8] Writing complete pinned BL2 partition image last.'
    preloader_size="$(file_size "$PRELOADER")"
...
usage() {
    cat <<'EOF'
Nokia XG-040G-MF installer MedveFlasher rc15
