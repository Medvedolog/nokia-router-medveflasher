...
run_check() {
    ...
    [ -z "$image" ] || { [ "$(sha_file "$image")" = "$SYSUPGRADE_SHA" ] || die 'production UBI sysupgrade SHA256 mismatch'; check_sysupgrade "$image"; }
    log ''
...
run_fullflash() {
    ...
    [ -f "$image" ] || die "sysupgrade image not found: $image"
    [ "$(sha_file "$image")" = "$SYSUPGRADE_SHA" ] || die 'production UBI sysupgrade SHA256 mismatch'
    log "FULLFLASH image: $image"
    log 'FULLFLASH: check -> format stock NAND as all-in-UBI -> embedded OpenWrt sysupgrade.'
    if [ "${NOKIA_AUTOMATIC_FULLFLASH:-0}" = 1 ]; then
