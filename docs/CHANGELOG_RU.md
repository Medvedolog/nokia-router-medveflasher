# 1.0.0-rc35 — MD preflight, RI MAC и eraseblock-aligned auto bundle

## 1.0.0-rc35 — 2026-08-25

- Исправлен полевой MD regression: общий `run_stage1()` больше не пишет `MF preflight не пройден` для XG-040G-MD; сообщение строится из фактического `profile.family`.
- Удалён забытый unconditional `RC32-prep1` stop, который блокировал destructive handoff любого MD уже после валидного backup. NAND vendor/model остаётся только INFO/WARN; write authority задают board/SoC, geometry, live MTD/UBI, pinned payload, backup и readback/hash.
- Fresh MD auto bundle из snapshot 2026-08-16 был `19955992` (`0x1308118`) байт и не делился на stock eraseblock `0x20000`. В релизе после точного embedded sysupgrade добавлено `98024` (`0x17ee8`) нулевых байт: итог `20054016` (`0x1320000`), SHA256 `ac9658f4d099ad0629a068ed579f8ed559857c0e1f151fa1dd6efc0268fb0b03`. FIT window, production offset `0x900000`, production size `10518808` и SHA256 `b0556660…8d867` не изменены. Alignment gate не ослаблялся.
...
- **Аварийный образ по умолчанию — transition-система, а не stock-recovery.**
  Плата попадает в `boot_tftp_forever` только когда BL2 и FIP уже записаны, то
  есть миграция состоялась. Полезный ответ там — единственный образ комплекта,
  несущий `nokia-ubi-installer` и `nokia-ubi-finish`: его инсталлятор видит
  существующие заголовки UBI, пишет *«refusing to format again automatically»*,
  идёт неразрушающим путём `attach_existing` и позволяет мастеру дописать
  production без повторной миграции. Комплект и так полагался на этот образ
  ровно в этой ситуации — именно он кладётся в том `fit` как пост-миграционный
  fallback. RC30 отдавал вместо него stock-recovery, который грузится на той же
  плате, но доделать установку не может, — и восстановимое устройство попадало
  не в ту систему.
...
- **Сообщения говорят, что делать дальше.** И строка запуска, и строка о
  доставке образа называют путь продолжения — поднять плату само по себе никогда
  не было целью.

- **Селфтест фиксирует дефолт по каждому семейству**, что `--stock-recovery`
  по-прежнему даёт откатный образ, что семейства не делят один образ и что
  отдаваемый файл равен заявленному размеру FIT. Проверено пятью намеренными
  регрессиями.

# 1.0.0-rc30 — установка носит аварийную страховку с собой

Полевая установка чисто завершила миграцию — `[1/8]`..`[8/8]`, BL2 записан,
fallback-FIT проверен в томе `fit` — затем вошла в production sysupgrade, и
больше от неё ничего не было. Мастер восемнадцать минут долбился в HTTP 80/443
и не сообщал ничего. Плата выглядела кирпичом.

Она им не была. Байты payload не менялись относительно RC24.

## 1.0.0-rc30 — 19 августа 2026

- **Аварийный TFTP-сервер держится всю стадию ожидания.** Штатный sysupgrade
  OpenWrt сначала удаляет том `fit` и только потом создаёт его заново
  (`nand_upgrade_prepare_ubi` → `ubirmvol -N fit` → `ubimkvol` → `ubiupdatevol`),
  поэтому между этими вызовами загрузочного образа на устройстве нет вообще.
  Прервавшись там, U-Boot делает ровно то, что записано у него в окружении:
  `boot_ubi = run boot_production ; run boot_tftp_forever` — и плата раз в
  секунду, бесконечно, просит у `serverip` файл `$bootfile`, со светящимся
  power-LED. Отвечать было некому. Теперь есть кому, и худший сценарий
  установки чинит себя сам, без единого действия оператора. Сервер поднимается
  до начала наблюдения и освобождается на любом выходе, включая Ctrl-C: он
  держит UDP/69, который позже в той же сессии нужен возврату на сток. ПК, не
  сумевший занять порт, всё равно ставит систему — это страховка, а не шлюз.
...
- **Свидетельства снимаются, пока устройство ещё отвечает.** Всё после
  `ubus call system sysupgrade` принадлежит procd; канал журнала пропадает, и
  отказ становится невидимым. Теперь в журнал сессии под меткой `[EVIDENCE]`
  попадают два read-only снимка — спокойный базовый, когда transition-система
  впервые отвечает, и best-effort в момент старта production sysupgrade — с
  `/proc/meminfo`, `df -k /tmp`, `/proc/mtd`, `ubinfo -a` и строками про
  UBI/ECC/бэд-блоки из `dmesg`. Именно это отличило бы исчерпание RAM от отказа
  на уровне NAND, и именно этого в логе не было.

- **Совет про перезагрузку перестал требовать того, чего оператор не видит.**
  Он требовал точной строки `sysupgrade successful` на UART — в инструменте,
  вся суть которого в отсутствии UART, — и предлагал снять питание на четвёртой
  минуте, то есть ровно внутри окна, где том `fit` может быть записан наполовину.
  Теперь на 90-й секунде выводится заметка об ожидаемом времени (~13 МиБ, штатно
  меньше минуты) и не предлагается ничего; на четвёртой минуте мастер объясняет,
  что делает procd, и формулирует совет по фактическому состоянию страховки.

- **Поток выдачи нельзя убить его собственной отчётностью.** Живой тест
  передачи — не селфтест — поймал вызов `tr()` внутри потока-спасателя: он
  разрешает язык интерфейса при первом обращении и спрашивает оператора, если
  язык не выбран. У фонового потока нет консоли, чтобы ответить, поэтому вызов
  бросал `EOFError` и уносил с собой весь цикл спасения — ровно в тот момент,
  ради которого он существует. Текст теперь разрешается на вызывающем потоке,
  переживает неудачу разрешения, а поток печатает уже готовую строку.

- **`_rc30_install_rescue_selftest`** фиксирует всё перечисленное: сервер создан,
  запущен и освобождён; оба снимка срабатывают; зонд остаётся read-only и не
  создаёт файлов на устройстве; сам захваченный вывод попадает в журнал сессии и
  никогда в консоль; совет читает состояние страховки; ранняя заметка сохраняет
  порог. Проверено восемнадцатью намеренными регрессиями — три из них вскрыли
  декоративные проверки в самом селфтесте, с тех пор переписанные.
