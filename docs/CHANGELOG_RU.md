...
- Шесть pinned runtime payload встроены и проверяются `tools/verify_release_assets.py` по размерам и SHA256 из `RELEASE_ASSETS_REQUIRED.md`. `verify_kit()` остаётся fail-closed: отсутствующий, изменившийся по размеру или не сошедшийся по SHA payload блокирует операцию.
