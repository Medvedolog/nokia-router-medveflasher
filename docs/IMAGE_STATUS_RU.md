# Статус образов Nokia Router MedveFlasher 1.0.0-rc29
...
| PC-side изменения RC21–RC29 | STATIC_QA_PASS, HW regression pending |
...
Firmware/transition/recovery payloads **не менялись с RC19**. RC20–RC29 меняют только PC-side orchestration, metadata и документацию, поэтому аппаратные подтверждения install-путей переносятся между этими релизами без пересборки образов.
