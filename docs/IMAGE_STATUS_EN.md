# Nokia Router MedveFlasher 1.0.0-rc29 image status
...
| PC-side changes RC21–RC29 | STATIC_QA_PASS, HW regression pending |
...
Firmware/transition/recovery payloads are **unchanged since RC19**. RC20–RC29 alter only PC-side orchestration, metadata, and documentation, so hardware evidence for the install paths carries across those releases without rebuilding images.
