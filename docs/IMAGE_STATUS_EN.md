# Nokia Router MedveFlasher 1.0.0-rc26 image status
...
| PC-side changes RC21–RC26 | STATIC_QA_PASS, HW regression pending |
...
Firmware/transition/recovery payloads are **unchanged since RC19**. RC20–RC26 alter only PC-side orchestration, metadata, and documentation, so hardware evidence for the install paths carries across those releases without rebuilding images.
