# 1.0.0-rc29 — возврат на сток умеет спросить пароль и перестал умирать по тайм-ауту

Возврат на сток по SSH с работающей OpenWrt падал на живом железе с сообщением
«SSH OpenWrt не стал устойчиво доступен». При этом SSH всё это время отвечал
корректно. Один и тот же тайм-аут давали два независимых дефекта. Байты payload
не менялись относительно RC24.

## 1.0.0-rc29 — 18 августа 2026

- **Цикл готовности больше не судит по размеру раздела UBI.**
  `wait_for_stable_openwrt` определял систему, сравнивая две целые строки
  `/proc/mtd` буквально, вместе с `mtd2: 0ffe0000`. Живое устройство публикует там
  `0ff00000` — семь eraseblock, которые другая сборка той же платы оставляет
  неиспользованными. Режим оставался неизвестным, цикл доходил до дедлайна, и
  восстановление сообщало, что SSH не стабилен. Теперь используется
  `_all_in_ubi_shape()` — та же структурная проверка, что и в шлюзе возврата
  после RC27: 256 МиБ `all_flash`, блок `bl2` размером `0x20000` и erase-размер
  `0x20000` проверяются точно, а размер раздела остаётся свидетельством, а не
  условием допуска. RC27 исправил это в шлюзе и не заметил цикл готовности.

- **Пароль root больше не убивает распознавание.** Оба детерминированных режима
  зонда идут с `BatchMode=yes` и получают `/dev/null` на stdin — это правильно
  для детектора, который обязан либо ответить, либо упасть, но не ждать молча.
  При первом контакте с *установленной* OpenWrt это превращало защищённый
  паролем root в тайм-аут: ssh было прямо запрещено спрашивать. Теперь одно
  распознавание, обращённое к оператору, может один раз перейти в интерактивный
  режим — только если зонд дошёл до SSH и отказала именно аутентификация, и
  только если к процессу подключена настоящая консоль. Все циклы опроса
  остаются неинтерактивными.

- **Один запрос пароля, а не десяток.** ssh нельзя попросить запомнить пароль,
  поэтому интерактивный зонд внутри цикла опроса означает по запросу на итерацию —
  дважды за вызов, три вызова на попытку перехода. Единственный интерактивный
  вход теперь в том же соединении кладёт одноразовый ключ ed25519 в
  `/etc/dropbear/authorized_keys`; все последующие зонды и все `scp` снова
  обычные batch-вызовы. Ключ генерируется на запуск во временный каталог,
  проверяется batch-зондом прежде чем ему поверят (и отбрасывается, если проверка
  не прошла), не попадает в журнал сессии и перезаписывается вместе со всей
  флеш-памятью самим возвратом на сток. Без `ssh-keygen` мастер работает
  по-прежнему, просто спросит пароль ещё раз.

- **Отказ говорит, что делать.** Когда отказала именно аутентификация, ошибка
  теперь называет три пути — снять пароль root (`passwd -d root`), добавить ключ
  в `/etc/dropbear/authorized_keys` или восстанавливать через BootROM/UART —
  вместо голого сообщения о тайм-ауте.

- **`_rc29_restore_ssh_auth_selftest` фиксирует всё перечисленное**: что
  запрашивать пароль вправе ровно одна точка вызова, что цикл готовности не может
  вернуть себе ни буквальную проверку размера, ни интерактивный зонд, и что
  непроверенный сессионный ключ отбрасывается, а не тащится дальше. Проверено
  семнадцатью намеренными регрессиями.

# 1.0.0-rc28 — RAM-worker обрёл собственный голос

Отсоединённый worker прошивки был RAM-автономен по исполнению и нем по отчётности:
его вывод уходил в `/dev/null`, а сам он поднимал пару `tee`/FIFO с того rootfs,
который собирался стереть. Байты payload не менялись относительно RC24.

## 1.0.0-rc28 — 18 августа 2026

- **Вывод worker'а идёт в обычный файл в tmpfs.** Он был перенаправлен в
  `/dev/null`, а полученные им параметры `ramlog`/`usb_log` присваивались и не
  использовались — поэтому каждая строка `[1/7]`..`[7/7]` и каждое сообщение
  `CRITICAL FAILURE` выбрасывались. Worker, стёрший `mtd14` и остановившийся, не
  оставлял ничего, что объяснило бы причину, — ровно наблюдавшийся инцидент, где
  стирание прошло, а записано не было ничего. Теперь stdout и stderr попадают в
  `$STAGE_RAM/flash.log`, так что сохраняется и то, что `mtd_debug` пишет в stderr.
- **В destructive-пути не работает никакой mirror.** Размещённый скрипт запускал
  `nokia_begin_output_mirror` для `--ram-flash`, а тому нужны `tee` и `mkfifo` —
  обе резолвятся по `PATH` со stock rootfs, то есть с `mtd3`, представления внутри
  стираемого `mtd14`. Канал умирал вместе с разделом, о котором должен был
  докладывать. Worker mirror не поднимает: его stdout уже открытый файл, и никакой
  pipeline, FIFO или внешний `nohup` не участвуют.
- **Worker больше не ссылается на каталог bundle.** Файл `stock-flash-medveflasher.log`
  в `$SCRIPT_DIR` лежал на stock NAND и никогда не писался; он удалён, а оператору
  указывается tmpfs-лог. Контракт `--ram-flash` лишился этого аргумента, поэтому
  запуск, диспетчер и worker сходятся на девяти.
- Случай в `selftest-safety` фиксирует инвариант — после первой destructive
  операции ни один executable, library, shell helper или logging primitive не
  берётся со stock NAND — и проверен четырьмя намеренными поломками: выброс
  вывода, возврат mirror, голый `sync` в worker и расхождение арности контракта.

# 1.0.0-rc27 — restore перестал охранять раздел, который не трогает

Найдено на железе: устройство с правильным именем платы, `all_flash` на 256 МиБ,
`bl2` в `0x20000` и разделом `ubi` размером `0x0FF00000` получало отказ при
откате на сток без UART. Байты payload не менялись относительно RC24.

## 1.0.0-rc27 — 18 августа 2026

- **`/proc/mtd` разбирается структурно, а не сличается текстом.** Гейт сравнивал
  строки целиком, поэтому `mtd2: 0ff00000` не сошёлся с `0x0FFE0000`, который
  публикует сборка этого комплекта, — это 7 eraseblock'ов, которые другая сборка
  той же платы оставляет в конце чипа неиспользованными. От этого числа в
  операции не зависит ничего: откат из работающей системы правит U-Boot
  environment, проверяет его обратным чтением, перезагружает устройство и отдаёт
  recovery-образ по TFTP в RAM, а раздел `ubi` при этом не читается, не стирается
  и не пишется; физическую геометрию recovery-система фиксирует сама до любой
  записи. Размер работал отпечатком личности в одежде проверки безопасности — та
  же подмена распознавания авторизацией, которую RC25 убрал из install-пути.
- Точным осталось то, что задано железом и загрузочным контрактом: весь чип
  256 МиБ как `all_flash`, один блок `0x20000` с именем `bl2`, erase `0x20000` и
  `mtd2` с именем `ubi` или `ibu`. Наблюдаемый размер раздела пишется в лог
  сессии как свидетельство `[RESTORE-SHAPE]`.
- **Личность платы читается отдельно от запущенной системы.** Зонд дополнительно
  сообщает `/proc/device-tree/compatible` — первичное свидетельство, несущее и
  SoC, — поэтому семейство MD опознаётся по `airoha,an7581` даже при пустом
  `board_name` или без суффикса `-ubi`. Суффикс сохранил своё более узкое
  значение: систему собрал этот комплект. Запущенная система классифицируется
  отдельно как `recovery`, `production`, `stock-layout` (transition этапа 1) или
  `foreign-ubi`.
- **Отказ теперь говорит, что он увидел.** Раньше на четыре разные ситуации
  отвечала одна фраза «не соответствует», пока наблюдаемые имя платы и
  `/proc/mtd` лежали неиспользованными в ответе зонда. Теперь они печатаются,
  называется несовпавшая половина, перечисляются недостающие маркеры и
  указывается подходящий путь: этап 2 для transition-системы, BootROM/UART для
  всего, что собрал не этот комплект. Разметка, совпадающая с all-in-UBI под
  именем платы без суффикса, называется осознанным решением, а не выдаётся за
  сбой распознавания.
- Selftests покрывают полевой размер, собственный размер комплекта, раздел
  recovery, MD с апстримным именем, опознание по одному SoC, разметку этапа 1 и
  четыре формы, которые обязаны отвергаться. Каждая проверена намеренной
  поломкой.

# 1.0.0-rc26 — консоль без часов, лог с часами

RC25 был опубликован и затем трижды пересобран под одним и тем же тегом. Этот
релиз существует, чтобы так больше не делать: содержимое, меняющее поведение,
получает свою версию, а `v1.0.0-rc25` остаётся на месте. Байты прошивочных
payload не менялись относительно RC24.

## 1.0.0-rc26 — 18 августа 2026

- **На консоли отметок времени нет вообще.** RC23 ввёл абсолютный префикс
  `[ГГГГ-ММ-ДД ЧЧ:ММ:СС]`, чтобы вывод ПК можно было сопоставить с событиями
  UART. Это работа с файлом, который читают потом, а не с экраном, за которым
  сидит оператор, — там префикс отнимал место у сообщения в каждой строке. Штамп
  переехал в `_ConsoleTee`, поэтому `work/logs/LATEST.log` и файл сессии несут
  время на каждой строке, включая пункты меню и приглашения ввода, а консоль
  показывает ровно то, что напечатал код.
- `_write_session_only()` пишет мимо tee и потому штампует свои диагностики сам;
  `_log_prompt_newline()` дополнительно сбрасывает позицию в логе, чтобы строка
  после приглашения не считалась продолжением и не теряла штамп.
- Консольная машинерия RC25 удалена: `menu_ui()`, `_MENU_RENDERING`,
  `_stamp_stream()` и `_timestamp_text()` убраны вместе со всеми двадцатью
  блоками `with menu_ui():`. Удаление проверено сравнением AST модуля до и после,
  так что исчезли только обёртки. Заменивший их инвариант проще и шире: консоль
  не штампуется никогда, лог — всегда.
- Штамп остаётся префиксом строки. Живые зеркала — посимвольный поток UART, эхо
  Telnet, счётчик XMODEM — пишут частичными кусками, и кусок, продолжающий
  строку, проходит нетронутым; перерисовка прогресса через `\r` не собирает
  штамп на каждое обновление.
- Случай в `selftest-safety` фиксирует обе половины разделения и проверен
  четырьмя намеренными поломками: консоль снова штампует, лог перестал
  штамповать, подавление вернулось, зеркало штампуется по кускам.
